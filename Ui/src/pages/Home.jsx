  ...
  <main className="flex-grow p-4">
      ...
      <div className="max-w-3xl mx-auto mb-8">
        <form action="/search" method="GET" className="w-full max-w-md flex">
          ...
          />
          <button type="submit" className="bg-blue-500 text-white rounded-r px-4 py-2" onClick={()=>setRoom(navigate(`/booking/${room}`))}>Search</button>
        </form>
      </div>
      ...
      <section className="max-w-3xl mx-auto">
        ...
        {booking.map(book => (
            <Booking key={book._id} book={book} />
          ))}
        </div>
      </section>
    </main>
